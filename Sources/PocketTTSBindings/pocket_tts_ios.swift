...
// The Rust static library ships arm64 slices only (device + arm64 Catalyst + arm64 simulator), so the
// bindings compile away on every other architecture — without this, universal (arm64 + x86_64) Mac
// Catalyst Release builds fail to link the x86_64 slice with undefined `_ffi_pocket_tts_ios_*` symbols.
// Re-apply this guard if the file is regenerated by uniffi.
#if arch(arm64)
